...
function configurePreflightResponse(req, res, options) {
    ...
    if (!checkOrigin_1.checkOrigin(req, options.origin)) {
        return;
    }
    ...
    if (util_1.isString(options.allowHeaders) && options.allowHeaders === '*') {
        headers.push({
        ...
        headers.push({
            key: applyHeaders_1.AccessControlHeader.Headers,
            value: options.allowHeaders.map(header => util_2.capitalize(header)).join(', '),
        });
    }
    if (options.maxAge) {
        headers.push({
    ...
    applyHeaders_1.applyHeaders(headers, res);
}
